...
function Popular({ refreshCart }) {
  ...
  const fetchData = async () => {
    try {
      ...
      const popularRes = await fetch('https://backend.frozenwala.com/api/recommended/');
  ...
  const handleScroll = (direction) => {
    ...
    setCurrentSlide((prev) =>
      direction === "next" ? (prev + 1) % totalSlides : (prev - 1 + totalSlides) % totalSlides
    );
  };
 ...
 <span
        ...
        {item.discount_percentage}% OFF
                          ...
                          {cartQty && !isSoldOut && isLoggedIn ? (
                            <div className="d-flex justify-content-center align-items-center gap-2">
                              <button className="qty-btn" onClick={() => subOne(item.id)}>
                                -
                              </button>
                              <span className="fw-bold">{cartQty}</span>
                              <button className="qty-btn" onClick={() => addOne(item.id)}>
                                +
                              </button>
                            </div>
                          ) : (
